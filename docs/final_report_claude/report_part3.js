...
// augmentation pipeline comparison with prior team, signer-disjoint
// splitting protocol, model architectures (3DCNN + SignVLM), and
// the full 6-stage experimental progression narrative.
// ================================================================
...
// ── helper: bold-italic inline label followed by plain body text ─
...
const methodologySection = [
  ...
  BR(),
  tblCaption("Table 2: PSL-104 dataset composition before augmentation. Clip counts are per-sign; totals are across all 104 classes."),
  new Table({
  ...
  body("The presence of these blank transition frames is particularly damaging for temporal models. A 3D-CNN or LSTM processing a clip that spends the first 20% of its frames on blank hand positioning and the last 15% on the hands returning to rest will necessarily attend to and partially model this non-informative content. Because transition postures are similar across many signs, this reduces the discriminative signal available to the model and adds noise to the temporal feature representations. In the worst case, a model might learn to key on the timing of transition frames relative to the signing window as a spurious feature, which would generalise poorly across signers who have different preparation speeds."),

  body("We addressed this by manually reviewing and temporally cropping every video in the dataset to a window of three to four seconds containing only the active signing motion. The cropping boundaries were set to the moment the signer's hands first begin to form the sign and the moment they complete the final position of the sign. This was carried out semi-automatically: a custom script identified candidate crop boundaries based on motion energy thresholding (flagging frames where pixel-level change exceeded a set threshold), and a team member then reviewed and corrected the boundaries for each clip. The final cropped clips range from approximately 90 to 120 frames at 30 fps, capturing the complete signing motion with minimal pre- and post-sign content."),
  ...
  body("where S_train, S_val, and S_test denote the sets of signers whose videos appear in the training, validation, and test splits respectively. This mirrors the Leave-One-Signer-Out (LOSO) evaluation protocol recommended as the minimum requirement for meaningful SLR results in the broader sign language recognition literature [12, 16, 17]."),

  body("Under this assignment, the training split contains recordings from four of the six signers (contributing fifteen original videos per class), the validation split contains recordings from one signer (contributing three original videos per class), and the test split contains recordings from the remaining signer (contributing one video per class plus the PSL Dictionary clips). The PSL Dictionary videos — recorded by the professional studio signer — are allocated to the test split only, as they represent the most controlled and standardised recordings and serve as a meaningful held-out evaluation set. This allocation is consistent across all experiments in the project."),
  ...
  H3("5.4.1 Spatial Augmentations (retained from prior pipeline, parameters refined)"),
  ...
  bull("Brightness-Saturation-Hue (BSH) transform: Adjusts HSV colour channels independently, simulating differences in camera white balance, sensor response across different devices, and variation in skin tone representation under different lighting conditions."),
  ...
  H3("5.4.3 Temporal Augmentations (new in our FYP-II pipeline)"),
  ...
  body("Table 3 provides a direct comparison of the prior team's augmentation pipeline and our redesigned pipeline, highlighting the additions and the net effect on dataset statistics."),

  BR(),
  tblCaption("Table 3: Augmentation pipeline comparison — FYP-I spatial-only approach vs. FYP-II spatial + temporal redesign."),
  new Table({
    width: { size: CONTENT, type: WidthType.DXA },
    columnWidths: [2200, 1480, 1480, 2100, 2100],
    rows: [
      new TableRow({ tableHeader: true, children: [
        hCell("Transform",            2200),
        hCell("FYP-I Pipeline",       1480),
        hCell("FYP-II Pipeline",      1480),
        hCell("Purpose",              2100),
        hCell("Diversity Introduced", 2100),
      ]}),
      // ── Spatial group ─────────────────────────────────────────
      new TableRow({ children: [
        grpCell("Spatial Transforms", 2200),
        grpCell("", 1480),
        grpCell("", 1480),
        grpCell("", 2100),
        grpCell("", 2100),
      ...
      ...[
        ...
        children: row.map((c, i) => dCell(c, [2200,1480,1480,2100,2100][i], false))
      })),
      // ── Photometric group ──────────────────────────────────────
      new TableRow({ children: [
        grpCell("Photometric Transforms", 2200),
        grpCell("", 1480),
        grpCell("", 1480),
        grpCell("", 2100),
        grpCell("", 2100),
      ...
      ...[
        ["Brightness adjust",  "Yes", "Yes", "Lighting intensity simulation",        "Indoor / outdoor lighting"],
      ...
      // ── Temporal group — NEW ───────────────────────────────────
      new TableRow({ children: [
        grpCell("Temporal Transforms  ★ NEW in FYP-II", 2200),
        grpCell("", 1480),
        grpCell("", 1480),
        grpCell("", 2100),
        grpCell("", 2100),
      ...
      ...[
        ["Frame jitter (1–3 frames)", "No", "Yes", "Clip boundary robustness",   "Sign onset timing variation"],
        ["Random frame drop (5–15%)", "No", "Yes", "Effective speed variation",  "Signing pace / speed diversity"],
      ].map(row => new TableRow({
        children: row.map((c, i) => {
          const w = [2200,1480,1480,2100,2100][i];
          // Grey-out the "No" cells; highlight the "Yes" cells in the new rows
          if (i === 1) return dCell(c, w, false, { color: "999999" });
          if (i === 2) return dCell(c, w, false, { bold: true, color: "1F6E38" });
          return dCell(c, w, false);
        })
      })),
      // ── Summary row ───────────────────────────────────────────
      new TableRow({ children: [
        bCell("Net effect on training set",                                    2200, false),
        dCell("Spatial diversity only\n10 → 60 clips / sign",                 1480, false),
        bCell("Spatial + temporal diversity\n15 → 45 clips / sign",           1480, false),
        bCell("Broader distribution coverage",                                 2100, false),
        bCell("More realistic inter-signer variation",                         2100, false),
      ]}),
    ]
  }),
  BR(),

  body("The net effect of the temporal augmentations is meaningful even though the raw clip counts after augmentation are numerically similar to the prior team's approach: the training split expands from 1,560 original clips to 4,680 augmented clips (thirty augmented variants per class). More importantly, the distribution of the augmented training data now covers temporal variation — signing pace, clip boundary sensitivity — in addition to spatial and photometric variation, producing a more realistic approximation of the variability a deployed model would encounter across new signers. Table 4 summarises the final dataset statistics after augmentation across all three signer-disjoint splits."),

  BR(),
  tblCaption("Table 4: PSL-104 dataset statistics after signer-disjoint splitting and augmentation."),
  new Table({
    width: { size: CONTENT, type: WidthType.DXA },
    columnWidths: [1700, 1100, 1700, 1700, 1360, 1800],
    rows: [
      new TableRow({ tableHeader: true, children: [
        hCell("Split",          1700),
        hCell("Classes",        1100),
        hCell("Original Clips", 1700),
        hCell("After Aug.",     1700),
        hCell("Avg. / Class",   1360),
        hCell("Signers",        1800),
      ]}),
      ...[
        ...
        children: row.map((c, i) => dCell(c, [1700,1100,1700,1700,1360,1800][i], true))
      })),
      new TableRow({ children: [
        bCell("Total",                  1700),
        bCell("104",                    1100),
  ...
  body("Two model architectures were employed throughout the experimental progression described in Sections 5.6–5.12: the 3DCNN + Residual Block + BiLSTM architecture inherited and extended from the prior FYP cohort, which serves as the controlled baseline across all comparison experiments; and SignVLM, a large pretrained visual-temporal model adapted to PSL-104 as the primary solution. This section describes both architectures in sufficient detail to understand the experimental results that follow."),

  H3("5.5.1 Baseline: 3DCNN + Residual Block + BiLSTM"),

  body("The 3DCNN + Residual Block + BiLSTM architecture was originally proposed by the prior FYP cohort and forms our controlled baseline. The architecture is designed to jointly extract spatiotemporal features from video clips and model the temporal dependencies across the frame sequence, without any reliance on pretrained weights. Its design drew inspiration from two prior works: the Data-Augmented Residual 3D CNN (DA-R3DCNN) proposed by Chen et al. (2023), which demonstrated that adding residual connections to 3D convolutional backbones improves gradient flow and enables deeper spatiotemporal feature learning; and the C3D-BiLSTM architecture with Multi-Head Attention proposed by Dey et al. (2024) for Wh-question sign recognition in ASL, which showed that bidirectional LSTMs more effectively capture signs where the temporal direction of the motion carries discriminative meaning."),
  ...
  body("The architecture is trained from random initialisation in all experiments — there are no pretrained weights of any kind. This is the critical distinction from SignVLM, and it is the primary reason the architecture struggles under signer-disjoint evaluation: without any pretrained visual prior, the model must learn to represent hand shapes and body configurations entirely from the limited PSL-104 training corpus."),

  BR(),
  figPlaceholder("Figure 2: 3DCNN + Residual Block + BiLSTM architecture. Three-stage pipeline: stacked Conv3D blocks → two Residual Blocks with skip connections → two-layer BiLSTM → linear classifier. Input shape (B, 3, T, H, W); output 104-class logits."),
  BR(),
  ...
  BR(),
  figPlaceholder("Figure 3: SignVLM two-component architecture. Frozen CLIP ViT-L/14 extracts per-frame CLS token embeddings (1,024-dim). The trainable EVL temporal decoder aggregates these across the T-frame sequence using temporal convolutions, cross-frame attention, and multi-head self-attention. A linear classifier produces 104-class logits."),
  BR(),

  body("The key advantage of SignVLM over scratch-trained architectures in this setting is precisely this property: CLIP's visual prior eliminates the need to learn what hands look like from PSL training data. The model only needs to learn which temporal patterns of CLIP-encoded features correspond to which PSL signs — a substantially simpler function to fit from fifteen clips per class than learning spatiotemporal representations entirely from scratch."),

  // ─────────────────────────────────────────────────────────────
  H2("5.6 Experimental Progression Overview"),
  // ─────────────────────────────────────────────────────────────

  body("Our experimental work proceeded through six stages, each motivated by the findings of the previous stage. Rather than presenting experiments in isolation, we describe them here in the order they were conducted, explaining the hypothesis that drove each experiment and what it revealed. This narrative is important because the experimental progression itself is a contribution of this project — it provides the first systematic empirical demonstration of the generalization gap in PSL SLR and documents a principled path toward closing it."),

  body("Table 5 provides an overview of all six stages with their key results; the following subsections describe each stage in detail."),

  BR(),
  tblCaption("Table 5: Experimental progression overview. All accuracy figures are Top-1. Eval. Protocol indicates whether signer-inclusive (random split) or signer-disjoint conditions were used. F1 is macro-averaged across 104 classes."),
  new Table({
    width: { size: CONTENT, type: WidthType.DXA },
    columnWidths: [340, 1620, 1340, 1500, 860, 860, 760, 1280],
    rows: [
      new TableRow({ tableHeader: true, children: [
        hCell("#",               340),
        hCell("Experiment",      1620),
        hCell("Model",           1340),
        hCell("Eval. Protocol",  1500),
        hCell("Top-1",           860),
        hCell("Top-5",           860),
        hCell("F1",              760),
        hCell("Key Finding",     1280),
      ]}),
      ...[
        ["1",  "Signer-Joint Baseline",        "3DCNN",    "Signer-inclusive",        "92.11%", "—",      "0.921",  "Pipeline confirmed; baseline extended"],
        ["2",  "Real-World Unseen Signers",    "3DCNN",    "Truly unseen (60 clips)", "8.33%",  "23.33%", "0.083",  "Generalization gap exposed empirically"],
        ["3a", "MediaPipe ROI (signer-joint)", "3DCNN+ROI","Signer-inclusive",        "45.46%", "~62%",   "~0.45",  "ROI cropping degrades, not improves"],
        ["3b", "MediaPipe ROI (disjoint)",     "3DCNN+ROI","Signer-disjoint",         "24.87%", "~38%",   "~0.25",  "Attention fix alone is insufficient"],
        ["4",  "Pose-Based SLR Exploration",   "N/A",      "N/A",                     "—",      "—",      "—",      "Deferred: tooling blockers (MMPose)"],
        ["5",  "Signer-Disjoint Baseline",     "3DCNN",    "Signer-disjoint",         "12.88%", "—",      "~0.13",  "Gap formally quantified: 83.89→12.88%"],
        ["6a", "SignVLM (16 frames)",          "SignVLM",  "Signer-disjoint",         "80.34%", "~91%",   "[TBD]",  "Gap largely closed by pretrained prior"],
        ["6b", "SignVLM (24 frames, aug.)",    "SignVLM",  "Signer-disjoint",         "83.49%", "~93%",   "[TBD]",  "Best augmented-data result"],
        ["6c", "SignVLM (24 frames, orig.)",   "SignVLM",  "Signer-disjoint",         "85.14%", "~94%",   "[TBD]",  "Best overall result (full-shot)"],
      ].map((row, idx) => {
        const isSignVLM = idx >= 6;
        const w = [340, 1620, 1340, 1500, 860, 860, 760, 1280];
        if (isSignVLM) return new TableRow({
          children: row.map((c, j) => bCell(c, w[j], j > 0))
        });
        return new TableRow({
          children: row.map((c, j) => dCell(c, w[j], j > 0))
        });
      })
    ]
  }),
  ...
  body("The prior team's 3DCNN + Residual Block + BiLSTM architecture was trained on the combined Phase 1 + Phase 2 dataset using a signer-inclusive random split — identical in methodology to the prior team's approach, but now drawing from all six signers rather than three. The same hyperparameters were used as in the prior team's training run: Adam optimiser with an initial learning rate of 1e-3, CrossEntropyLoss, training for 10 epochs with a ReduceLROnPlateau scheduler. The prior team's spatial-only augmentation pipeline was also used for this run to ensure a fair like-for-like comparison before our redesigned pipeline was introduced."),

  body("The results confirmed that the additional signer data produced a meaningful improvement: the signer-joint test accuracy improved from the prior team's 83.89% to 92.11%, with Precision 0.9367, Recall 0.9211, F1 0.9207, and Test Loss 0.3459. This improvement is exactly what one would expect when adding more training examples: the model had access to more variation in how each sign is performed across different individuals, making it better able to generalise across the visual differences between the six training signers. The result also validated that our combined preprocessing pipeline was correct and that the dataset was properly organised."),

  body("It is critical to re-emphasise what this result means: 92.11% accuracy under signer-inclusive evaluation is a strong result, but it reflects the model's ability to classify video clips from signers it has already seen during training. Whether this performance would hold on a genuinely new signer is an entirely separate question — and Stage 2 answers it directly."),
  ...
  body("The Stage 1 result was promising in isolation, but it had a fundamental limitation: the model had been evaluated on video clips from the same six signers it was trained on. To determine whether the model had actually learned sign-discriminating features — or had merely learned to recognise the training signers' individual appearances — we conducted a real-world inference test using video clips from individuals who had never appeared in the dataset at all."),

  body("We collected 60 short video clips covering a subset of the 104 PSL sign classes, recorded by friends and acquaintances of the team who had no prior involvement in the project. These individuals were asked to perform specific PSL signs based on the same PSL Dictionary reference videos that informed the original dataset recordings, so the sign content was as accurate as participants could manage without professional sign language training. The 60 clips were preprocessed using the same pipeline as the training data: .mov to .mp4 conversion, audio removal, temporal cropping to the active signing window, and frame pre-extraction. Inference was then run using the Stage 1 signer-joint trained 3DCNN model without any retraining."),

  body("The results were stark. The model correctly classified only 5 of 60 clips at Top-1 — an accuracy of 8.33% — and 14 of 60 clips at Top-5, giving a Recall@5 of 23.33%. The complete metrics were: Accuracy 8.33%, Precision 0.0833, Recall 0.0833, F1 0.0833. For context, random chance for a 104-class classification problem gives approximately 0.96% Top-1 accuracy. The model therefore performed only marginally above chance on genuinely new signers, despite achieving 92.11% on the known-signer test set."),

  body("This 83.78 percentage-point gap between signer-inclusive test accuracy (92.11%) and real-world unseen-signer accuracy (8.33%) is the central empirical finding that motivates everything else in this project. It confirms that the signer-inclusive evaluation prevalent in the PSL SLR literature does not reflect real-world model capability, and that any system reporting only signer-inclusive accuracy figures cannot be considered deployable. The model had not learned PSL signs — it had learned to recognise the training signers."),

  body("The failure mode is well-understood in the SLR literature. With approximately fifteen training clips per class, a scratch-trained 3D-CNN has insufficient data to learn robust visual features from first principles. The model is forced to leverage the easiest available discriminative signal, which in a signer-inclusive training corpus is not the sign's gestural content but the individual signer's appearance: their background, their skin tone relative to the background, their hand size, their face, their clothing colour. These identity-specific features are stable within a signer's training clips and appear in both the training and the signer-inclusive test set, making them effective class discriminators under that evaluation condition. They are completely useless for classifying clips from new individuals."),

  BR(),
  figPlaceholder("Figure 5: Stage 2 — Generalization gap visualization. Bar chart comparing 92.11% signer-joint test accuracy vs. 8.33% unseen-signer inference accuracy for the same 3DCNN model. The 83.78 percentage-point gap quantifies the effect of signer-identity leakage in the training and evaluation protocol."),
  BR(),
  ...
  body("The diagnosis from Stage 2 suggested a clear hypothesis: the model was attending to whole-frame appearance features rather than focusing on the hands, where the sign's linguistic content is actually encoded. If we could force the model to look only at the hand region — by cropping each video to just the bilateral hand bounding box before training — the signer-specific background and body appearance cues would be eliminated from the input, and the model might be compelled to learn genuine hand-configuration features instead."),
  ...
  body("The ROI extraction pipeline operated as follows. For each frame of each video, MediaPipe's palm detector was run to locate the bounding boxes of all detected hands. The bounding boxes of the left and right hands were merged into a single bilateral bounding box covering both hands (their union), to preserve relative hand positioning for two-handed signs. A padding factor of 1.3 was applied to expand the bounding box by 30% in each direction, ensuring that the full hand including fingertips was captured even when the palm detector's bounding box was slightly tight. The padded bilateral crop was then resized to a fixed output resolution. In frames where MediaPipe failed to detect any hands — due to motion blur, fast motion, occlusion, or the hand being temporarily outside the detector's range — a fallback was applied: the full frame at reduced resolution was written rather than dropping the frame, to preserve temporal continuity."),

  body("This pipeline was applied to all videos across all three dataset splits, producing parallel ROI-cropped video directories. The 3DCNN architecture was then retrained from scratch on the ROI-cropped dataset under both signer-inclusive and signer-disjoint evaluation conditions."),

  body("The results were counterproductive. Under signer-inclusive evaluation, accuracy fell dramatically from 92.11% (full-frame, Stage 1) to 45.46% (ROI-cropped). Under signer-disjoint evaluation, accuracy was 24.87% — meaningfully higher than the 12.88% signer-disjoint full-frame result from Stage 5, but far below practical utility. Post-hoc analysis identified four reasons for this failure:"),

  num("MediaPipe failure rate under fast motion. Sign language involves rapid hand movements that frequently exceed MediaPipe's frame-to-frame tracking capability. When hands move quickly, the detector loses track and falls back to the full frame for that segment, producing inconsistent crops across the clip — some frames show the tight hand ROI, others show the full frame. This temporal inconsistency is disruptive to temporal models that expect spatially stable input across frames."),
  num("Aspect ratio distortion for two-handed signs. When both hands are far apart — a common configuration in two-handed signs — the bilateral union bounding box becomes very wide relative to its height. Resizing this to a fixed-aspect-ratio output distorts the spatial relationships between the two hands, potentially obscuring the configuration information that distinguishes the sign."),
  num("No pretrained visual prior. ROI cropping changes the input to the 3DCNN but does not add any new visual knowledge. The model still has to learn what a hand looks like from fifteen training clips per class — the fundamental data scarcity problem is unchanged. Removing the background shortcut without providing an alternative source of discriminative features simply left the model with a harder learning problem and less total discriminative signal."),
  num("Loss of non-manual cues. Sign language relies partly on facial expression and body posture to convey grammatical meaning. By cropping to the hand region, the model loses access to these non-manual features entirely. Even in the signer-inclusive setting, some discriminative information was evidently being drawn from the whole-frame context, and losing it produced the large accuracy drop from 92.11% to 45.46%."),

  body("These findings are not discouraging — they are instructive. They reveal that the generalization problem in PSL SLR is not simply a spatial attention problem that can be solved by restricting the model's input field of view. The root cause is the absence of a pretrained visual prior: the model needs to start from rich, pre-learned representations of what hands look like and how they move before it can extract sign-specific temporal patterns from fifteen clips per class. This insight directly motivated the adoption of SignVLM."),

  BR(),
  figPlaceholder("Figure 6: Stage 3 — MediaPipe ROI extraction examples. Left column: original full frames. Right column: corresponding ROI-cropped frames. Rows 1–2 show successful crops on clear, well-lit, slow-motion frames. Rows 3–4 show failure cases: a fast-motion frame where MediaPipe lost tracking and fell back to the full frame, and a two-handed sign where the bilateral union crop produces a highly distorted wide aspect ratio."),
  BR(),
  ...
  body("Concurrent with the MediaPipe ROI experiments, we explored a fundamentally different approach to the signer generalization problem: pose-based SLR. The key theoretical motivation is that skeletal pose representations — sequences of 3D joint positions describing the signer's hands and body — are by construction independent of appearance features. A pose sequence encodes only the geometry of the signing motion, not the background, skin tone, clothing, or any other identity-specific visual attribute. Models operating in pose space therefore cannot, in principle, exploit signer identity as a classification shortcut, making them inherently more signer-agnostic."),

  body("We explored two architectures from the pose-based SLR literature as potential candidates. SignBERT (Hu et al., 2021) and its successor SignBERT+ (Hu et al., 2023) both propose masked pose reconstruction as a self-supervised pretraining strategy for sign language representations, analogous to BERT's masked language modelling objective applied to skeletal sequences. The pretraining phase — which masks a subset of joints and trains a transformer to predict the masked configurations — produces rich representations of signing motion that can then be fine-tuned on small labeled corpora. This is precisely the kind of pretrained prior we were seeking in the pose domain."),

  body("For pose estimation itself — the step of converting raw RGB video frames to 3D joint keypoints — we evaluated two options. MediaPipe Pose provides a lightweight body landmark estimation pipeline, but its hand keypoint precision degrades significantly under fast motion and partial occlusion, particularly for the 21-point per-hand keypoint map required for fine-grained hand-shape discrimination. MMPose, the pose estimation framework from the OpenMMLab research ecosystem, provides access to state-of-the-art pose estimation models with substantially higher accuracy than MediaPipe under challenging conditions."),

  body("Unfortunately, MMPose could not be stably installed in either the Windows development environment or the Linux compute environment available to the project. The MMPose installation process involves a complex dependency chain — including specific versions of MMCV, MMEngine, and their associated CUDA extensions — that produced irreconcilable configuration conflicts in our hardware and OS combination. Multiple installation attempts with different version pinnings all failed to produce a working environment within the project timeline. MediaPipe Pose, while installable, did not provide the keypoint quality needed for hand-shape-discriminating recognition in our fast-motion signing videos, as already evidenced by the MediaPipe ROI results in Stage 3."),

  body("As a result, the pose-based SLR direction was deferred rather than abandoned. The theoretical argument for pose-based approaches remains strong — combining CLIP's spatial visual prior with a pose-based temporal representation is a promising direction for future work. We return to this in Section 10 (Future Work). The practical blockers encountered here further reinforced the motivation for the CLIP-based approach: SignVLM's large-scale pretraining implicitly learns attention to hands and body structure without requiring an explicit, fragile, and hardware-sensitive pose estimation step."),
  ...
  body("The training configuration was updated from Stage 1 to adopt best-practice techniques for training from scratch on small datasets. The Adam optimiser was replaced with AdamW, which decouples weight decay from the gradient update step and provides better regularisation on limited data. The initial learning rate was reduced from 1e-3 to 1e-4. Label smoothing of 0.04 was applied to reduce overconfident softmax probabilities. A cosine learning rate decay schedule with a 3-epoch warmup replaced the ReduceLROnPlateau scheduler. Gradient clipping (max norm 1.0) was applied to stabilise LSTM training, and WeightedRandomSampler was used to correct for minor class imbalances in the training distribution. Mixed-precision training (AMP, float16) was enabled to reduce GPU memory usage and training time. Early stopping was configured with a patience of 12 epochs."),

  body("The model was trained for 30 epochs before early stopping triggered, at which point the best validation accuracy seen during training was 12.88% — achieved at epoch 18. For reference, random chance for a 104-class problem gives 0.96% accuracy. The model therefore learned meaningfully above chance, but the margin is too small to be practically useful. The training and validation loss curves were nearly flat throughout, hovering around log(104) ≈ 4.644 nats — the theoretical cross-entropy of a uniform distribution over 104 classes — confirming that no meaningful class-discriminating learning occurred under the signer-disjoint condition."),

  body("The contrast between 83.89% (prior team, signer-inclusive, same architecture class) and 12.88% (our team, signer-disjoint) formally quantifies the effect of signer leakage in prior PSL SLR work. This is not a critique of the prior team's results — 83.89% is the correct answer to the question their evaluation asked. It is a demonstration that the question their evaluation asked is not the right question for assessing real-world system utility."),

  BR(),
  figPlaceholder("Figure 7: Stage 5 — Signer-Disjoint 3DCNN training curves over 30 epochs. Both training and validation loss remain near the theoretical entropy floor of log(104) ≈ 4.644 nats throughout training, confirming that no meaningful class-discriminating learning occurs when signer identity is unavailable as a shortcut. Best validation accuracy: 12.88% at epoch 18."),
  BR(),
  ...
  num("Frame sampling: 24 frames per clip were uniformly sampled from each video. This matches the optimal frame count identified in the original SignVLM paper for datasets with comparable temporal complexity to PSL — longer clips (32 frames) showed marginal or negative returns due to redundant frames, while shorter clips (16 frames) slightly underperformed on signs requiring broader temporal context."),
  num("Frozen encoder: The CLIP ViT-L/14 encoder was kept fully frozen throughout all training runs. Experiments with partial unfreezing showed training instability when encoder layers were updated on the small PSL corpus, consistent with the well-documented risk of catastrophic forgetting when fine-tuning large pretrained models on limited data."),
  ...
  num("Offline frame pre-extraction: As described in Section 5.2.3, all video frames were pre-extracted to JPEG files before training, reducing data loading time by approximately 16× and maintaining GPU utilization above 85% throughout the training runs."),

  body("Three configurations of SignVLM were evaluated under strictly signer-disjoint conditions: 16 frames per clip on the augmented training set; 24 frames per clip on the augmented training set; and 24 frames per clip on the original (non-augmented) training clips, the last of which also served as the basis for the N-shot data efficiency ablation. Full results with complete metrics are presented in Section 7."),
